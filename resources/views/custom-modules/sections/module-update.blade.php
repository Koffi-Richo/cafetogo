@if ($fetchSetting?->purchase_code && $fetchSetting?->supported_until)
    @if (\Carbon\Carbon::parse($fetchSetting->supported_until)->isPast())
        <button type="button" class="btn btn-outline-secondary btn-sm refreshModule f-11"
                data-toggle="tooltip"
                data-original-title="This will fetch the latest support date from tdm_code. Click on this button only when you have renewed the support and the new support date is not reflecting"
